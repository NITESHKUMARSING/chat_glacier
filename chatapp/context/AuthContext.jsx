...
//   // ✅ Setup axios defaults on initial load
//   useEffect(() => {
//     if (token) {
...
//   // ✅ Sync token with localStorage when changed
//   useEffect(() => {
//     if (token) {
...
const backendUrl = import.meta.env.VITE_BACKEND_URL; // ✅ use env correctly
...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (state, credentials) => {
    try {
      ...
      if (data.success) {
        ...
        axios.defaults.headers.common["Authorization"] = authHeader; // ✅ fixed
        setToken(data.token);
        localStorage.setItem("token", data.token);
        toast.success(data.message);
      } else {
  ...
  useEffect(() => {
    if (token) {
      ...
      checkAuth(); // ✅ only after setting token
    } else {
      delete axios.defaults.headers.common["Authorization"];
    }
  }, [token]); // ✅ token change dependency
